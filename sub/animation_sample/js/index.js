...
/////////////////////////////////////////////////////////////
//애니메이션
$(".ani_op8").stop().mouseenter(function(){
   $(this).animate({'opacity':0,'top':'30vh'},1000);
setTimeout(function(){
    $(".ani_op8").css({'opacity':1,'top':'20vh'});
},5000);
});
...
$(btn3).on('click',function(){
    $(".demo3").show();
    $(".demo3 > div").hide();
    var ex_sel = document.getElementById("example_sel");
    var ex_idx = ex_sel.options.selectedIndex;
    //나타나기
    if(ex_idx == 0){
        $(".ex_op1").stop().show();  
    }
...
$(function () {
    ...
    //     $(".result").show();
    //     var randIdx = Math.floor(Math.random()*2); //0~1사이의 난수를 생성하고 2배를 곱해 1이 넘거나 안넘거나인데 결과값에서 나머지 소수정을 floor()로 버리면 0혹은 1의 랜덤값이 나온다.
    //     if(randIdx == 0){
    //         $(".result").text("꽝");
    //     }
    //     else{
    ...
    // ********* 당첨 확률을 30퍼센트로 조절하고싶은 경우 ********
    $(".ex_op2_list").find('li').on('click',function(){
        $(".ex_op2_result").show();
        var randIdx = parseInt(Math.random()*100);
        if(randIdx <= 30){
            $(".ex_op2_result").text("당첨");
        }
        else{
            $(".ex_op2_result").text("꽝");
        }
    });

    $(".ex_op2_result").on('click',function(){
        $(this).hide();
    });
